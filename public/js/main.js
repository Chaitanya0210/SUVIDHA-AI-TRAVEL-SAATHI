...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Attach AI Planner Form Listener
...
// -----------------------------------------------------------------------------
function initMap() {
  // Center map globally over India / Asia initial coordinates
  ...
  // Set default pin
  currentMarker = L.marker([20.5937, 78.9629]).addTo(map)
    .bindPopup('<b>SUVIDHA AI SAATHI Map Engine</b><br>Select or generate a trip to view destination markers.')
    .openPopup();
}
...
function renderDestinationsGrid(items) {
  ...
  if (!items || items.length === 0) {
    grid.innerHTML = `<div style="grid-column: 1/-1; text-align: center; color: var(--text-muted); padding: 3rem;">No destinations match your filter criteria.</div>`;
    return;
  }
    ...
    return `
      <div class="glass-panel card">
        <div class="card-img-wrap">
          <img src="${d.imageUrl}" alt="${d.name}" class="card-img" onerror="this.src='https://images.unsplash.com/photo-1488646953014-85cb44e25828?w=800'">
        ...
        <div class="card-body">
          ...
          <div style="display: flex; gap: 0.4rem; flex-wrap: wrap; margin-bottom: 1rem;">
            ${(d.travelVibes || []).map(v => `<span style="font-size: 0.75rem; background: rgba(255,255,255,0.06); padding: 0.2rem 0.6rem; border-radius: 12px; color: var(--accent-cyan);">${v}</span>`).join('')}
          </div>

          <div class="card-meta">
            <div class="card-price">$${d.estimatedCostPerDay} <span>/ day est.</span></div>
            <div style="display: flex; gap: 0.5rem;">
              ...
              <button class="btn btn-primary" onclick="viewDestinationDetail('${d.name}')">Details</button>
            </div>
          </div>
        </div>
      </div>
...
async function handleAiPlannerSubmit(e) {
  ...
  try {
    ...
    if (json.status === 'success') {
      ...
      if (plan.coordinates) {
        updateMapMarker(plan.coordinates.lat, plan.coordinates.lng, plan.destinationName, `Custom ${plan.durationDays}-Day ${plan.travelVibe} Itinerary`);
      }
      showToast(`Generated AI Trip for ${plan.destinationName}!`, 'success');
      
      // Scroll to workspace map area
  ...
  output.innerHTML = `
    <div style="border-bottom: 1px solid var(--border-color); padding-bottom: 1rem; margin-bottom: 1.5rem;">
      <div style="display: flex; justify-content: space-between; align-items: center;">
        <h2 style="font-size: 1.6rem;">${plan.destinationName}, ${plan.country || ''}</h2>
      ...
      <div style="display: flex; gap: 2rem; margin-top: 1rem; background: rgba(255,255,255,0.04); padding: 0.8rem 1rem; border-radius: 8px;">
        <div>
          <span style="font-size: 0.75rem; color: var(--text-muted); display: block;">ESTIMATED TOTAL</span>
          <strong style="color: var(--accent-emerald); font-size: 1.2rem;">$${plan.estimatedTotalCost} USD</strong>
        </div>
        <div>
          <span style="font-size: 0.75rem; color: var(--text-muted); display: block;">TRAVEL VIBE</span>
          <strong style="color: var(--accent-cyan); font-size: 1.05rem;">${plan.travelVibe}</strong>
        </div>
      </div>
    </div>

    <div>
      <h3 style="font-size: 1.2rem; margin-bottom: 1rem;">Day-by-Day Schedule</h3>
      ${plan.itinerary.map(day => `
        <div class="itinerary-day-card">
          ...
          <div style="margin-top: 0.75rem; padding-top: 0.5rem; border-top: 1px dashed var(--border-color); font-size: 0.85rem; color: var(--text-muted); display: flex; justify-content: space-between;">
            <span><strong>Stay Recommendation:</strong> ${day.stay}</span>
            <span style="color: var(--accent-amber); font-weight: 600;">~$${day.estimatedDayCost}/day</span>
          </div>
        </div>
      `).join('')}
    </div>
...
function viewDestinationDetail(name) {
  ...
  content.innerHTML = `
    ...
    <h2 style="font-size: 1.8rem; margin-bottom: 0.5rem;">${dest.name}, ${dest.country}</h2>
    <p style="color: var(--text-secondary); margin-bottom: 1rem;">${dest.description}</p>
    
    <div style="display: flex; gap: 1rem; margin-bottom: 1rem; background: rgba(255,255,255,0.05); padding: 1rem; border-radius: 10px;">
      <div><strong>Category:</strong> ${dest.category}</div>
      <div><strong>Best Season:</strong> ${dest.bestSeasons ? dest.bestSeasons.join(', ') : 'All Year'}</div>
      <div><strong>Avg Cost:</strong> $${dest.estimatedCostPerDay}/day</div>
    </div>
    ...
    <ul style="padding-left: 1.2rem; margin-bottom: 1.5rem; color: var(--text-secondary);">
      ${(dest.topAttractions || ['Local Scenic Viewpoints', 'Historic Town Center', 'Artisan Markets']).map(a => `<li>${a}</li>`).join('')}
    </ul>
    ...
    <button class="btn btn-primary" onclick="closeModal('detail-modal'); document.getElementById('destination-input').value='${dest.name}'; handleAiPlannerSubmit(new Event('submit'));" style="width: 100%; justify-content: center;">
      <i class="fa-solid fa-wand-magic-sparkles"></i> Generate Custom AI Itinerary for ${dest.name}
    </button>
  `;
...
async function handleAuthSubmit(e) {
  ...
  try {
    ...
    if (json.status === 'success') {
      ...
      showToast(`Welcome ${json.data.name}!`, 'success');
  ...
  if (user && navItem) {
    navItem.innerHTML = `
      <div style="display: flex; align-items: center; gap: 0.75rem;">
        <span style="font-size: 0.9rem; color: var(--accent-emerald);">👋 ${user.name}</span>
